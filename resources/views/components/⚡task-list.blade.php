...
<div class="max-w-4xl mx-auto py-8">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-gray-600 mt-1">{{ __('Welcome back, :param_1! Here are your tasks.', ['param_1' => auth()->user()->name]) }}</p>
        </div>
                        ...
                        <td class="px-4 py-3 text-sm text-gray-600">
                            @if ($task->due_date)
                                @if ($task->due_date < now() && $task->status !== 'completed')
                                    <span class="text-red-600 font-medium">{{ __('Overdue: :param_1', ['param_1' => $task->due_date->diffForHumans()]) }}</span>
                                @else
